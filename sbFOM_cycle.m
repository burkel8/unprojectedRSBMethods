% Function which runs a single cycle of the shifted block Full Orthogonalization Method (sbFOM).
...
function [r,X,R,H,W, nmv] = sbFOM_cycle(A,X,R,shifts,m,s,n)

nmv = 0;  %variable to store total number of vectors A is applied to throughout the cycle.
Y = zeros(m*s,s);
E = zeros(m*s,s);
...
%compute norm of updated block residual
r = norm(R);
end